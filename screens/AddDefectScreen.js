  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isCentralSyncEnabled()) {
        try {
          await pushDefectToCentral(defectData);
          syncMessage = '\nSynced to central storage.';
        } catch (syncError) {
          console.error('Central sync failed:', syncError);
          syncMessage = '\nSaved on this device. Central sync will retry when network is available.';
        }
      }
      
      Alert.alert(
        'Success',
        `Defect recorded successfully!\nDefect ID: ${defectId}${syncMessage}`,
        [
          {
            text: 'OK',
            onPress: () => {
