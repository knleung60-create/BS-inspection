...
A mobile application for Android devices designed to help building inspection teams record and track defects efficiently. Data is stored locally on the device and can optionally sync to a NAS-hosted central storage service.
...
- **Local Storage**: Data stored securely on the device using SQLite
- **NAS Central Sync**: Optional central defect and photo storage for multiple users
...
- **Central Sync**: Configure `EXPO_PUBLIC_SYNC_SERVER_URL` to sync defects to a NAS server
- **Privacy**: Without sync configuration, all data remains local

## NAS Central Sync

See [CENTRAL_SYNC_GUIDE.md](CENTRAL_SYNC_GUIDE.md) for setup details.
